...
then
    ...
    then
        if [ "$2 " == "" ]
        then
            echo "Specify a word and definition!"
        elif [ "$3" == "" ]
        then
            echo "Please specify a definition!"
        else
            echo "$2" >> "$wordpath"
            echo "$3" >> "$answerpath"
            wordnum=$(("$wordnum" +1))
    ...
    then
        if [ "$wordnum" -lt 5 ]
        then
            echo "The number of words is too small to propperly play multiple choice mode. Please add more numbers!"
        else
            while [ "$i" != 0 ]
            do
